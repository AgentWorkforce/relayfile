...
| --- | --- | --- |
| `activity-summary` | Implemented with guarded edges | `today.md`, `yesterday.md`, date-stamped daily files, weekly windows, mounted `/.skills/activity-summary.md`, and `by-edited/<date>` fallback aliases are present across the runtime and priority adapters. Large cloud windows emit warning metadata rather than silently claiming completeness. |
| `daily-digest` | Implemented | Digest rendering is generic upstream over workspace events; adapters expose metadata and aliases rather than provider-owned digest bullet handlers. CLI rebuild supports `today`, `yesterday`, `YYYY-MM-DD`, `this-week`, and `last-week`. |
| `writeback-as-files` | Implemented | File-native writeback, schemas, examples, dead-lettering, retry, provider execution, and Notion markdown write-through exist; state-listing is exposed through CLI/SDK paths as they land. |
| `workspace-layout` | Implemented with compatibility alias | Root `LAYOUT.md` and canonical `<provider>/LAYOUT.md` are exposed in local and hosted surfaces; legacy `<provider>/.layout.md` remains a read-compatible alias in the local mount. Priority adapters emit `by-edited/<date>` fallback aliases. |
...
- `relayfile` mount FUSE layer exposes virtual `LAYOUT.md` and
  canonical `<provider>/LAYOUT.md`, plus legacy `<provider>/.layout.md`
  compatibility reads.
...
| Gap | Repo(s) | Notes |
| --- | --- | --- |
| Provider layout path standardization | `relayfile`, `cloud`, `relayfile-adapters`, `skills` | Resolved: `<provider>/LAYOUT.md` is canonical; relayfile keeps `<provider>/.layout.md` as a compatibility alias only. |
| Root layout mentions digests and skills directory | `relayfile`, `cloud` | Resolved for root layouts generated by runtime surfaces. |
| `by-edited/<date>/` indexes | `cloud`, `relayfile-adapters`, `relayfile` | Resolved for priority provider resources used by activity-summary fallback: GitHub issues/pulls, Linear issues, Notion pages, Jira issues, and Confluence pages. |
