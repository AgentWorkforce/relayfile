...
project mount. `agent-relay cloud login` remains a compatible recovery path for
machines that already use that CLI. The running mount continues serving local
reads from disk until it is restarted.
