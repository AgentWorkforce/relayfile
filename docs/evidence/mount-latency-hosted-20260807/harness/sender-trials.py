...
def request(server, workspace, endpoint, payload, token, correlation_id):
    ...
    # The hosted deployment's Cloudflare policy rejects Python's default
    # urllib user agent (error 1010) before the application sees the request.
    # Pin a stable harness identity; this changes no payload or timestamp.
    req.add_header("User-Agent", "relayfile-latency-harness/1")
    if method == "PUT":
