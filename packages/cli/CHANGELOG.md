...
- Mount bootstrap stalls now terminate the CLI daemon after the persisted retry limit, survive supervisor restarts without reissuing the same request, and surface as `mount: stalled` with the blocked path. Bootstrap progress uses the tree API's authoritative file total when safe, suppresses totals that include pruned runtime subtrees, and never renders a synthetic `/0` denominator.
