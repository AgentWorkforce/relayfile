...
- `relayfile-mount --once` no longer reports a fatal `mount sync cycle failed` (exit 1) when a per-cycle deadline lands on a workspace that already tracks files but has not completed its bootstrap — the "detected non-empty state without completed bootstrap" recovery shape. The public `.relay/state.json` `bootstrap` block is only published once a full pull has started, so the previous classification ignored the authoritative private state and aborted Cloud's bootstrap-owner flush on a workspace that was still legitimately bootstrapping. A per-cycle deadline while the authoritative bootstrap is incomplete is now reported as `mount bootstrapping: bootstrap incomplete (in progress)` and the run exits 0, leaving the persisted state for the next bounded resume instead of failing the run.
...
[0.10.31]: https://github.com/AgentWorkforce/relayfile/releases/tag/v0.10.31
