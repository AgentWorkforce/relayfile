...
const relayfileCLIMountBlockerEnv = "RELAYFILE_CLI_TEST_MOUNT_BLOCKER"
...
func TestMountDaemonBlocker(t *testing.T) {
	if os.Getenv(relayfileCLIMountBlockerEnv) != "1" {
		t.Skip("helper process")
	}
	select {}
}

func startMountDaemonBlocker(t *testing.T) int {
	t.Helper()
	cmd := exec.Command(os.Args[0], "-test.run=^TestMountDaemonBlocker$")
	cmd.Env = append(os.Environ(), relayfileCLIMountBlockerEnv+"=1")
	if err := cmd.Start(); err != nil {
		t.Fatalf("start mount blocker: %v", err)
	}
	t.Cleanup(func() {
		if cmd.Process != nil {
			_ = cmd.Process.Kill()
...
func TestMountBackgroundSpawnFailureLeavesTopologyForDaemonizedChild(t *testing.T) {
	skipUntilScopedOperatorSurfacesReady(t)
	t.Setenv("HOME", t.TempDir())
	clearRelayfileEnv(t)
	...
	if _, ok := workspaceRecordByID("ws_demo"); ok {
		t.Fatal("background parent mutated topology before the daemonized child acquired its lifetime lease")
	}
	if _, statErr := os.Stat(localRoot); !errors.Is(statErr, os.ErrNotExist) {
		t.Fatalf("failed background parent initialized mirror before child startup: %v", statErr)
	}
}
...
func TestEnsureMirrorLayoutTreatsScopedWorkspaceRootAsCanonical(t *testing.T) {
	localRoot := filepath.Join(t.TempDir(), "mirror")
	if err := ensureMirrorLayoutForTopology(localRoot, mountscope.LayoutScoped, "/"); err != nil {
		t.Fatalf("prepare Cloud-compatible scoped root: %v", err)
	}
	if info, err := os.Stat(filepath.Join(localRoot, mountscope.DigestsTopLevel)); err != nil || !info.IsDir() {
		t.Fatalf("canonical root digest directory: info=%v err=%v", info, err)
	}
	skill := filepath.Join(localRoot, mountscope.SkillsTopLevel, "activity-summary.md")
	if info, err := os.Stat(skill); err != nil || info.IsDir() {
		t.Fatalf("canonical root activity skill %s: info=%v err=%v", skill, info, err)
	}
}
...
func TestValidateMountResetModeRefusesScopedAcknowledgedReset(t *testing.T) {
	...
	if err := validateMountResetMode(localRoot, mountscope.LayoutScoped, true, "/"); err != nil {
		t.Fatalf("root-compatible scoped layout should retain acknowledged reset, got %v", err)
	}
	if err := validateMountResetMode(localRoot, mountscope.LayoutScoped, true, "/github"); err == nil {
		t.Fatal("isolated scoped child should still refuse non-transactional reset")
	}
}
...
func TestMountRefusesCompetingDaemonBeforePersistingAddedScope(t *testing.T) {
	...
	blockerPID := startMountDaemonBlocker(t)
	if err := writeDaemonPIDState(mountPIDFile(localDir), daemonPIDState{
		PID:         blockerPID,
		WorkspaceID: "ws_demo",
		LocalDir:    localDir,
...
func TestScopedWorkspaceConsumersAggregateChildRuntimeState(t *testing.T) {
	...
	for index, scope := range scopes {
		...
		listenerStatus := []string{"listening", "retrying"}[index]
		state := fmt.Sprintf(
			`{"status":"scope-%d","lastReconcileAt":"%s","lastSuccessfulReconcileAt":"%s","pendingWriteback":99,"failedWritebacks":%d,"stallReason":"scope-%d stalled","incrementalReadNotReadySince":{"event":"now"},"bootstrap":{"phase":"pull","filesSynced":%d,"filesTotal":10},"eventListener":{"mode":"websocket","status":"%s"}}`,
			index+1,
			reconcileAt,
			reconcileAt,
			index+3,
	...
	if snapshot.Status != "scope-1; scope-2" {
		t.Fatalf("aggregate sync status = %q, want both scoped child states", snapshot.Status)
	}
	...
	statusPayload, err := json.Marshal(workspaceSyncStatusJSON(snapshot, record))
	if err != nil {
		t.Fatal(err)
	}
	var scopedStatus scopedWorkspaceSyncStatus
	if err := json.Unmarshal(statusPayload, &scopedStatus); err != nil {
		t.Fatal(err)
	}
	if scopedStatus.Readiness != "pull" || len(scopedStatus.Scopes) != 2 {
		t.Fatalf("scoped status readiness = %q scopes=%#v, want attributable child readiness", scopedStatus.Readiness, scopedStatus.Scopes)
	}

	health := buildWorkspaceHealthReport("ws_demo", record)
	if health.Status != "scope-1; scope-2" ||
		health.Readiness != "pull" ||
	...
	if health.EventListener != nil || len(health.Scopes) != 2 ||
		health.Scopes[0].EventListener == nil || health.Scopes[0].EventListener.Status != "listening" ||
		health.Scopes[1].EventListener == nil || health.Scopes[1].EventListener.Status != "retrying" {
		t.Fatalf("scoped listener readiness was not attributable to both child roots: %#v", health)
	}
...
func TestScopedWorkspaceViewReadsOwningChildReadiness(t *testing.T) {
	localRoot := t.TempDir()
	githubRoot := mountscope.LocalDir(localRoot, "/github")
	slackRoot := mountscope.LocalDir(localRoot, "/slack")
	if err := writeMirrorStateFile(githubRoot, syncStateFile{
		WorkspaceID: "ws_demo",
		Status:      "ready",
		EventListener: &syncStateEventListener{
			Mode:   "websocket",
	...
	if err := writeMirrorStateFile(slackRoot, syncStateFile{
		WorkspaceID: "ws_demo",
		Status:      "ready",
		EventListener: &syncStateEventListener{
			Mode:   "websocket",
	...
	viewDir := filepath.Join(t.TempDir(), "slack-view")
	if err := os.Symlink(slackRoot, viewDir); err != nil {
		t.Fatal(err)
	}
	record := workspaceRecord{
		ID:          "ws_demo",
	...
	health := buildWorkspaceHealthReport("ws_demo", record)
	if len(health.Views) != 1 || health.Views[0].Status != "not-listening" {
		t.Fatalf("scoped view did not use owning child readiness: %#v", health.Views)
	}
	if health.Views[0].EventListener == nil || health.Views[0].EventListener.Status != "retrying" {
		t.Fatalf("scoped view listener = %#v, want slack child retrying", health.Views[0].EventListener)
	}
}
