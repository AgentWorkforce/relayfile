  ...
  const result = await workflow('063-agent-workspace-golden-path')
    ...
    .step('commit-cloud', {
      ...
      command: [
        ...
        `cd ${shell(CLOUD_ROOT)}`,
        'git status --short',
        'git add "packages/web/app/api/v1/workspaces/[workspaceId]/integrations/[provider]/status/route.ts" "packages/web/app/api/v1/workspaces/[workspaceId]/join/route.ts" packages/web/lib/workspace-registry.ts tests/sdk-setup-client-routes.test.ts tests/agent-workspace-golden-path-routes.test.ts 2>/dev/null || true',
        'git diff --cached --name-only',
    ...
    .step('commit-relaycast', {
      ...
      command: [
        ...
        `cd ${shell(RELAYCAST_ROOT)}`,
        'git status --short',
        'git add docs/sdk-setup-client.md packages/sdk-typescript/src/__tests__/invite-consumption.test.ts 2>/dev/null || true',
        'git diff --cached --name-only',
