...
const result = await workflow('relayfile-ci-and-publish')
  ...
  .step('read-sdk-package', {
    type: 'deterministic',
    command: `cat ${RELAYFILE}/sdk/relayfile-sdk/package.json 2>/dev/null || echo "no package.json"`,
    captureOutput: true,
  })
...
1. **ci.yml** — runs on every PR and push to main:
   ...
   - TS SDK: cd sdk/relayfile-sdk && npm ci && npm run build && npx tsc --noEmit
...
- Jobs:
  ...
  3. sdk-typecheck: setup Node 22, cd sdk/relayfile-sdk, npm ci, npm run build, tsc --noEmit
...
- Steps:
  ...
  3. Install deps: cd sdk/relayfile-sdk && npm ci
...
Update ${RELAYFILE}/sdk/relayfile-sdk/package.json:
...
Also create/update ${RELAYFILE}/sdk/relayfile-sdk/tsconfig.json:
{
  "compilerOptions": {
    "target": "ES2022",
    "module": "ESNext",
...
Create ${RELAYFILE}/sdk/relayfile-sdk/README.md with:
- Package name + description
- Install: npm install @relayfile/sdk
- Quick example: create client, list tree, read/write file
- Link to full docs

Verify it builds: cd sdk/relayfile-sdk && npm run build
...
cd sdk/relayfile-sdk && npm install 2>&1 | tail -1 && npm run build 2>&1 | tail -3; echo "EXIT: $?"`,
    captureOutput: true,
    failOnError: false,
  })
