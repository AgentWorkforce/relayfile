...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "generic_webhook_ingest":
		s.handleGenericWebhookIngest(w, r, workspaceID, correlationID)
...
// handleGenericWebhookIngest handles POST /v1/workspaces/{workspaceId}/webhooks/ingest
// Accepts generic webhook payloads for any provider
func (s *Server) handleGenericWebhookIngest(w http.ResponseWriter, r *http.Request, workspaceID, correlationID string) {
	...
	if !ok || provider == "" {
		writeError(w, http.StatusBadRequest, "bad_request", "missing or invalid 'provider' field", correlationID)
		return
	}

	eventType, ok := payload["event_type"].(string)
	if !ok || eventType == "" {
		writeError(w, http.StatusBadRequest, "bad_request", "missing or invalid 'event_type' field", correlationID)
		return
	}

	path, ok := payload["path"].(string)
	if !ok || path == "" {
		writeError(w, http.StatusBadRequest, "bad_request", "missing or invalid 'path' field", correlationID)
		return
	}

	// Extract optional fields
	deliveryID := ""
	if did, ok := payload["delivery_id"].(string); ok {
		deliveryID = did
	}
	...
	// Build the payload for the envelope
	// The payload should include event_type, path, and data at top level for generic parsing
	envelopePayload := make(map[string]any)
	envelopePayload["event_type"] = eventType
	envelopePayload["path"] = path
	...
	// Create WebhookEnvelopeRequest with provider name
	req := relayfile.WebhookEnvelopeRequest{
		EnvelopeID:    "", // Will be generated by store
		WorkspaceID:   workspaceID,
		Provider:      provider,
...
// handleWritebackPending handles GET /v1/workspaces/{workspaceId}/writeback/pending
// Returns pending writeback items from the queue
func (s *Server) handleWritebackPending(w http.ResponseWriter, r *http.Request, workspaceID, correlationID string) {
	items := s.store.GetPendingWritebacks(workspaceID)
	writeJSON(w, http.StatusOK, items)
}

// handleWritebackAck handles POST /v1/workspaces/{workspaceId}/writeback/{id}/ack
// Acknowledges a writeback item as processed
func (s *Server) handleWritebackAck(w http.ResponseWriter, r *http.Request, workspaceID, itemID, correlationID string) {
	var payload map[string]any
	if !s.decodeJSONBody(w, r, correlationID, &payload) {
		return
	}
