...
func TestGithubTarballImportEnforcesPathScopesForContentAndMarker(t *testing.T) {
	store := relayfile.NewStoreWithOptions(relayfile.StoreOptions{DisableWorkers: true})
	t.Cleanup(store.Close)
	server := NewServer(store)
	ownerToken := mustTestJWT(t, "dev-secret", "ws_tarball_scope", "Owner", []string{"fs:read", "fs:write"}, time.Now().Add(time.Hour))
	limitedToken := mustTestJWT(t, "dev-secret", "ws_tarball_scope", "Limited", []string{"relayfile:fs:write:/github/repos/octo/allowed/**"}, time.Now().Add(time.Hour))
	...
	resp := doRawRequest(t, server, rawRequest{
		method: http.MethodPost,
		path:   "/v1/workspaces/ws_tarball_scope/fs/import/github-tarball?owner=octo&repo=demo&headSha=abc123&jobId=job-scope",
		headers: map[string]string{
			"Authorization":    "Bearer " + limitedToken,
			"Content-Type":     "application/gzip",
			"X-Correlation-Id": "corr_tarball_scope_import",
		},
		body: archive,
	})
	if resp.Code != http.StatusOK {
		t.Fatalf("expected 200 with per-file scope errors, got %d (%s)", resp.Code, resp.Body.String())
	}
	var payload githubTarImportResponse
	if err := json.NewDecoder(resp.Body).Decode(&payload); err != nil {
		t.Fatalf("decode import response: %v", err)
	...
	if payload.ErrorCount != 2 || len(payload.Errors) != 2 {
		t.Fatalf("expected content and marker scope errors, got %+v", payload.Errors)
	}
	errorPaths := map[string]string{}
	for _, item := range payload.Errors {
		errorPaths[item.Path] = item.Message
	}
	if errorPaths["/github/repos/octo/demo/contents/README.md"] != "file access denied by path scope" {
		t.Fatalf("expected content path-scope error, got %+v", payload.Errors)
	}
	if errorPaths["/github/repos/octo/demo/.relayfile/clone.json"] != "file access denied by path scope" {
		t.Fatalf("expected marker path-scope error, got %+v", payload.Errors)
	}
	...
	for _, path := range []string{
		...
		readResp := doRequest(t, server, request{
			method: http.MethodGet,
			path:   "/v1/workspaces/ws_tarball_scope/fs/file?path=" + path,
			headers: map[string]string{
				"Authorization":    "Bearer " + ownerToken,
				"X-Correlation-Id": "corr_tarball_scope_read",
			},
		})
		if readResp.Code != http.StatusNotFound {
			t.Fatalf("expected %s to remain unwritten, got %d (%s)", path, readResp.Code, readResp.Body.String())
