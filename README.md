...
`--remote-path`. With `--local-layout scoped`, each subtree is mirrored under
the matching path inside the local root, which avoids full-workspace export
pulls on large workspaces:
